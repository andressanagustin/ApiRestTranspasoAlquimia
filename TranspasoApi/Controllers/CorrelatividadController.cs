...
{
    public class CorrelatividadController : BaseController
    {
        [HttpPost]
        ...
        private Reply TrabajaDatos(TablasViewModels _model, string _method = "POST")
        {
            ...
            try
            {
                ...
                GuardarAuditoria(true, "Tabla: CORRELATIVIDAD");
            }
            catch (Exception ex)
            {
        ...
        private bool Agregar(List<object> _listData) {
            using (var db = new dbAlumnado())
            {
                ...
                {
                    var obj = JsonConvert.DeserializeObject<al_correlatividad>(item.ToString());
                    db.al_correlatividad.Add(obj);
                }
                db.SaveChanges();
            }
            return true;
