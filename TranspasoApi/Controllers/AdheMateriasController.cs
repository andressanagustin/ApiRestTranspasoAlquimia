...
{
    public class AdheMateriasController : BaseController
    {
        [HttpPost]
        ...
        private Reply TrabajaDatos(TablasViewModels _model, string _method = "POST")
        {
            ...
            try
            {
                ...
                GuardarAuditoria(true, "Tabla: ADHERENTES MATERIAS");
            }
            catch (Exception ex)
            {
        ...
        private bool Agregar(List<object> _listData) {
            using (var db = new dbAlumnado())
            {
                ...
                {
                    var obj = JsonConvert.DeserializeObject<al_adhe_materias>(item.ToString());
                    db.al_adhe_materias.Add(obj);
                }
                db.SaveChanges();
            }
            return true;
