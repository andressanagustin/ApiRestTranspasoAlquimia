...
{
    public class NotaParcialController : BaseController
    {
        [HttpPost]
        ...
        private Reply TrabajaDatos(TablasViewModels _model, string _method = "POST")
        {
            ...
            try
            {
                ...
                GuardarAuditoria(true, "Tabla: NOTA PARCIAL");
            }
            catch (Exception ex)
            {
        ...
        private bool Agregar(List<object> _listData) {
            using (var db = new dbAlumnado())
            {
                ...
                {
                    var obj = JsonConvert.DeserializeObject<al_nota_parcial>(item.ToString());
                    db.al_nota_parcial.Add(obj);
                }
                db.SaveChanges();
            }
            return true;
