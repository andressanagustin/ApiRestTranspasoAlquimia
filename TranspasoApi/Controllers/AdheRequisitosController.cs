...
{
    public class AdheRequisitosController : BaseController
    {
        [HttpPost]
        ...
        private Reply TrabajaDatos(TablasViewModels _model, string _method = "POST")
        {
            ...
            try
            {
                ...
                GuardarAuditoria(true, "Tabla: ADHERENTES REQUISITOS");
            }
            catch (Exception ex)
            {
        ...
        private bool Agregar(List<object> _listData) {
            using (var db = new dbAlumnado())
            {
                ...
                {
                    var obj = JsonConvert.DeserializeObject<al_adhe_requisitos>(item.ToString());
                    db.al_adhe_requisitos.Add(obj);
                }
                db.SaveChanges();
            }
            return true;
