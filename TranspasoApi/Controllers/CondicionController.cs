    ...
    public class CondicionController : BaseController
    {
        ...
        private Reply TrabajaDatos(TablasViewModels _model, string _method = "POST")
        {
            ...
            try
            {
                ...
                GuardarAuditoria(true, "Tabla: CONDICION");
            }
            catch (Exception ex)
            {
        ...
        private bool Agregar(List<object> _listData) {
            using (var db = new dbAlumnado())
            {
                ...
                {
                    var obj = JsonConvert.DeserializeObject<t_condicion1>(item.ToString());
                    if (obj.cod_tipo != null && obj.cod_tipo.Trim() != "")
                    {
                        db.t_condicion1.Add(obj);
                    }
                }
                db.SaveChanges();
            }
